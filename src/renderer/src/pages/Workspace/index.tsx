...
export default function Workspace() {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      {/* Workspace header */}
      <div className="titlebar-drag shrink-0 flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-white/[0.07]">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                viewMode === v.id
                  ...
                  : 'text-gray-500 dark:text-white/45 hover:text-gray-700 dark:hover:text-white/75 hover:bg-black/[0.04] dark:hover:bg-white/[0.06]'
              }`}
            >
              {v.icon}
              {v.label}
          ...
          <div className="w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            <div className="bg-white dark:bg-[#1a2233] rounded-2xl shadow-2xl border border-gray-200 dark:border-white/[0.1] overflow-hidden">
              <div className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-100 dark:border-white/[0.06]">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className="text-gray-400 dark:text-white/30 shrink-0">
                ...
                />
                <kbd className="text-[10px] text-gray-400 dark:text-white/25 px-1.5 py-0.5 rounded bg-gray-100 dark:bg-white/[0.06] border border-gray-200 dark:border-white/[0.08]">Esc</kbd>
              </div>
              <div className="max-h-72 overflow-y-auto">
                {searchResults.length === 0 ? (
                  <p className="text-sm text-gray-400 dark:text-white/30 text-center py-8">
                  ...
                  <button key={task.id} onClick={() => { selectTask(task); setShowSearch(false); setSearchQuery('') }}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {task.client && <p className="text-xs text-gray-400 dark:text-white/30 mt-0.5 truncate">{task.client}</p>}
                    </div>
                    <span className="text-[10px] text-gray-400 dark:text-white/30 shrink-0">{CONTENT_TYPE_LABELS[task.content_type]}</span>
                  </button>
                ))}
              </div>
              {searchQuery === '' && (
                <div className="px-4 py-3 border-t border-gray-100 dark:border-white/[0.05] flex gap-4 text-[10px] text-gray-400 dark:text-white/20">
