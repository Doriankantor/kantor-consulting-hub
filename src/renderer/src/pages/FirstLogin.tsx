    ...
    <div className="h-screen flex items-center justify-center bg-hub-navy p-4 overflow-hidden">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {([1,2,3] as Step[]).map(s => (
            <div key={s} className={`h-1.5 rounded-full transition-all ${s === step ? 'w-8 bg-hub-gold' : s < step ? 'w-5 bg-hub-gold/50' : 'w-5 bg-white/[0.12]'}`} />
          ))}
        </div>

        <div className="text-center mb-6">
          <p className="text-[10px] text-white/25 uppercase tracking-widest mb-1.5">{steps[step - 1]}</p>
          {step === 1 && (
            <>
              <h1 className="text-xl font-bold text-white">Set your password</h1>
              <p className="text-white/40 text-sm mt-1.5">Welcome, {localUser?.name}. Create a permanent password to continue.</p>
            </>
          )}
          {step === 2 && (
            <>
          ...
          {step === 3 && (
            <>
              ...
              <p className="text-white/40 text-sm mt-1.5">Set your notification preferences.</p>
            </>
          )}
        </div>

        <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-6 shadow-2xl shadow-black/40">
          {step === 1 && (
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-[10px] font-semibold text-white/35 uppercase tracking-widest mb-1.5">Temporary password</label>
                <input type="password" value={currentPw} onChange={e => setCurrentPw(e.target.value)} autoFocus
                  placeholder="From your invite email"
                  className="titlebar-no-drag w-full px-3.5 py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.1] text-white placeholder-white/25 text-sm focus:outline-none focus:ring-2 focus:ring-hub-gold/40 transition" />
              </div>
              <div>
          ...
          {step === 3 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between py-1">
                <div>
                  <p className="text-sm text-white/80 font-medium">Email notifications</p>
                  <p className="text-xs text-white/35 mt-0.5">Receive updates when tasks are assigned to you</p>
                </div>
                <button
                  onClick={() => setEmailNotif(v => !v)}
                  className={`titlebar-no-drag relative w-11 h-6 rounded-full transition-colors ${emailNotif ? 'bg-hub-gold' : 'bg-white/[0.12]'}`}
                >
