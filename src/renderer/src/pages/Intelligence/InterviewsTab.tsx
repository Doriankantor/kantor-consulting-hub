...
const OPENCARDS_KEY = 'intel-opencards-interviews'
...
export default function InterviewsTab({ onApprove, project = null }: Props) {
  ...
  // on a non-empty list (interviews is [] before load — pruning then would wipe the store)
  // and no-ops (same ref) when nothing is stale, so it never clobbers present entries.
  useEffect(() => {
    if (interviews.length === 0) return
    setOpenCards(prev => {
      const ids = new Set(interviews.map(iv => iv.id))
  ...
  const load = useCallback(async (opts?: { background?: boolean; append?: boolean }) => {
    ...
    const filters = { type: 'interview', excludeStatus: 'routed', ...(project?.id ? { project: project.id } : {}) }
  ...
  // Interviews load with confidence=null; the shared ConfidencePill renders a neutral "set
  // confidence" affordance and handleConfidence persists the researcher's first pick (no fake LOW).
  async function handleCountries(id: string, subject: string[], mentioned: string[], subGeo: Record<string, string[]>) {
    ...
    if (!res?.ok) {
      console.error('[InterviewsTab] updateCountries failed — state not updated:', res?.error ?? 'no result')
      return
    }
    setInterviews(prev => prev.map(s => s.id === id ? {
      ...s,
  ...
  async function handleActors(id: string, next: { name: string; type: string }[]) {
    ...
    setInterviews(prev => prev.map(s => s.id === id ? { ...s, actors: JSON.stringify(next) } : s))
  ...
  async function handleConfidence(id: string, confidence: string) {
    ...
    setInterviews(prev => prev.map(s => s.id === id ? { ...s, confidence: confidence as any, confidence_override: 1 } : s))
  }
  ...
  async function handleIncident(id: string, value: boolean) {
    ...
    } catch (e) { console.warn('[InterviewsTab] setIncidentFlag failed:', e) }
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
        ...
        {!loading && visible.map(iv => {
          ...
          const core = fromIntelligenceSource(iv)
          ...
          return (
            <div key={iv.id} className="bg-white dark:bg-white/[0.04] rounded-xl border border-gray-200 dark:border-white/[0.08] p-4">
              ...
              <SourceCard
                core={core}
                onCountriesChange={(subject, mentioned, subGeo) => handleCountries(iv.id, subject, mentioned, subGeo)}
                onActorsChange={next => handleActors(iv.id, next)}
                onConfidenceChange={v => handleConfidence(iv.id, v)}
                onIncidentChange={v => handleIncident(iv.id, v)}
                onTagsChange={nextTags => handleSetTags(iv.id, nextTags)}
                tagVocabulary={knownThematic}
                onTagCreate={name => handleCreateTag(iv.id, themaTags, name, projectBoardSel)}
                ...
                geoTouched={countriesTouched.has(iv.id)}
                actorsTouched={actorsTouched.has(iv.id)}
              />
              ...
              <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-white/[0.06]">
                ...
                <button
                  ...
                  title={!online ? 'Unavailable while offline' : !projectBoardSel ? 'Select a project first' : !gate.all ? 'Not routable yet — complete the checklist (the dots on the card)' : 'Route this interview into the project’s New sources'}
