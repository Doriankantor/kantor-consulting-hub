...
export default function Clients() {
  ...
  const inputCls = 'titlebar-no-drag w-full px-2.5 py-2 rounded-lg bg-gray-50 dark:bg-white/[0.05] border border-gray-200 dark:border-white/[0.08] text-gray-900 dark:text-white text-sm placeholder-gray-400 dark:placeholder-white/25 focus:outline-none focus:ring-1 focus:ring-hub-gold/40'
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-80 shrink-0 flex flex-col border-r border-gray-200 dark:border-white/[0.08] bg-white dark:bg-black/10 overflow-hidden">
        ...
        <div className="px-4 pt-5 pb-3 border-b border-gray-100 dark:border-white/[0.06]">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-white/10 text-gray-500 dark:text-white/40 text-[10px] font-semibold">
                {clients.length}
              </span>
            </div>
            {isAdmin && (
          ...
          <div className="relative mb-2">
            <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 dark:text-white/30" width="12" height="12" viewBox="0 0 12 12" fill="none">
            ...
            <input
              ...
              className="titlebar-no-drag w-full pl-8 pr-3 py-1.5 rounded-lg bg-gray-100 dark:bg-white/[0.06] border border-gray-200 dark:border-white/[0.07] text-gray-900 dark:text-white text-xs placeholder-gray-400 dark:placeholder-white/30 focus:outline-none focus:ring-1 focus:ring-hub-gold/30"
            />
          </div>
              ...
              <button
                ...
                className={`titlebar-no-drag px-2 py-0.5 rounded-full text-[10px] font-semibold transition ${
                  filterType === t
                    ...
                    : 'bg-gray-100 dark:bg-white/[0.06] text-gray-500 dark:text-white/40 hover:bg-gray-200 dark:hover:bg-white/[0.10]'
                }`}
              >
                {t || 'All'}
              </button>
        ...
        {showNewClient && isAdmin && (
          <div className="px-4 py-4 border-b border-gray-100 dark:border-white/[0.06] space-y-2.5 bg-gray-50 dark:bg-white/[0.02]">
            <p className="text-[10px] font-semibold text-gray-400 dark:text-white/40 uppercase tracking-widest">New Client</p>
            ...
            <textarea value={newClient.notes} onChange={e => setNewClient(p => ({ ...p, notes: e.target.value }))} placeholder="Notes (optional)" rows={2}
              className="titlebar-no-drag w-full px-2.5 py-2 rounded-lg bg-gray-50 dark:bg-white/[0.05] border border-gray-200 dark:border-white/[0.08] text-gray-900 dark:text-white text-sm placeholder-gray-400 dark:placeholder-white/25 focus:outline-none focus:ring-1 focus:ring-hub-gold/40 resize-none"
            />
            {areas.length > 0 && (
              <div>
                <p className="text-[10px] text-gray-400 dark:text-white/35 mb-1.5">Area tags</p>
                <div className="flex flex-wrap gap-1">
                  {areas.map(a => {
                    ...
                    return (
                      <button
                        ...
                        className={`titlebar-no-drag px-2 py-0.5 rounded-full text-[10px] font-medium border transition ${sel ? 'text-white' : 'bg-gray-100 dark:bg-white/[0.06] text-gray-500 dark:text-white/40 border-gray-200 dark:border-white/[0.08]'}`}
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { setShowNewClient(false); setFormMsg(null) }}
                className="titlebar-no-drag px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-white/[0.06] text-gray-500 dark:text-white/45 text-xs transition">
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-32 text-gray-400 dark:text-white/30">
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-[#0f1623]">
        {!selectedClient ? (
          <div className="flex flex-col items-center justify-center flex-1 text-gray-400 dark:text-white/25">
                  ...
                  {editingClient ? (
                    ...
                    <>
                      <button onClick={startEdit}
                        className="titlebar-no-drag px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-white/[0.08] hover:bg-gray-200 dark:hover:bg-white/[0.12] text-gray-600 dark:text-white/60 hover:text-gray-900 dark:hover:text-white text-xs font-medium transition">
                        Edit
                      </button>
                  ...
                  {editingClient ? (
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <p className="text-[10px] font-semibold text-gray-400 dark:text-white/40 uppercase tracking-widest mb-1">Type</p>
                          <select value={editData.type ?? ''} onChange={e => setEditData(p => ({ ...p, type: e.target.value }))} className={selectCls}>
                            {CLIENT_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                          </select>
                        </div>
                        <div>
                          <p className="text-[10px] font-semibold text-gray-400 dark:text-white/40 uppercase tracking-widest mb-1">Status</p>
                        ...
                        <div>
                          <p className="text-[10px] font-semibold text-gray-400 dark:text-white/40 uppercase tracking-widest mb-1">Country</p>
                          <input value={editData.country ?? ''} onChange={e => setEditData(p => ({ ...p, country: e.target.value }))} placeholder="Country" className={inputCls} />
                        </div>
                        <div>
                          <p className="text-[10px] font-semibold text-gray-400 dark:text-white/40 uppercase tracking-widest mb-1">Region</p>
                          <input value={editData.region ?? ''} onChange={e => setEditData(p => ({ ...p, region: e.target.value }))} placeholder="Region" className={inputCls} />
                        </div>
                      </div>
                      <div>
                        <p className="text-[10px] font-semibold text-gray-400 dark:text-white/40 uppercase tracking-widest mb-1">Notes</p>
                      ...
                      {areas.length > 0 && (
                        <div>
                          <p className="text-[10px] font-semibold text-gray-400 dark:text-white/40 uppercase tracking-widest mb-1.5">Area Tags</p>
                    ...
                    <>
                      ...
                      {(detail.client.country || detail.client.region) && (
                        <div>
                          <p className="text-[10px] font-semibold text-gray-400 dark:text-white/40 uppercase tracking-widest mb-1.5">Location</p>
                      ...
                      {detail.client.notes && (
                        <div>
                          <p className="text-[10px] font-semibold text-gray-400 dark:text-white/40 uppercase tracking-widest mb-1.5">Notes</p>
                      ...
                      {detail.client.primary_contact_name && (
                        <div>
                          <p className="text-[10px] font-semibold text-gray-400 dark:text-white/40 uppercase tracking-widest mb-1.5">Primary Contact</p>
              ...
              {activeTab === 'contacts' && (
                <div className="max-w-xl space-y-3">
                  ...
                  {detail.contacts.length === 0 && !showNewContact && (
                    <p className="text-sm text-gray-400 dark:text-white/30 italic py-4 text-center">No contacts yet.</p>
                  )}
                    ...
                    {detail.contacts.map(c => (
                      <div key={c.id} className="flex items-start gap-3 p-3 rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.06] group">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {c.phone && <p className="text-xs text-gray-400 dark:text-white/35 mt-0.5">{c.phone}</p>}
                        </div>
                        {isAdmin && (
                          <button
              ...
              {activeTab === 'tasks' && (
                <div className="max-w-xl">
                  {detail.tasks.length === 0 ? (
                    <p className="text-sm text-gray-400 dark:text-white/30 italic py-4 text-center">No tasks linked to this client.</p>
                  ) : (
                    <div className="space-y-2">
                      {detail.tasks.map(t => (
