...
interface Window {
  api: {
    ...
    intelligence: {
      ...
      analyzeText:          (opts: { task: 'relevance' | 'reconcile'; text: string; projectConfig?: { name?: string; keywords?: string; scope?: string } | null; userNotes?: string | null; existingTags?: string[]; priorAi?: Record<string, unknown> | null }) => Promise<{ ok: true; result: { relevance_score?: number; relevance_reasoning?: string; summary?: string; suggested_tags?: string[]; article_type?: string; capabilities?: Array<{ system: string; actor?: string; actor_type?: string; cost?: string; category?: string; relationship?: string }>; key_facts?: Array<{ label: string; value: string }>; proposed_sections?: Array<{ section: string; confidence: 'high' | 'medium' | 'low' }>; channel?: 'state-procurement' | 'commercial-retail' | 'n/a'; routing_reasoning?: string } } | { ok: false; error: string }>
