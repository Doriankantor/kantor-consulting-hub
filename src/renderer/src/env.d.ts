...
interface IntelligenceSource {
  ...
  actors?: string | null               // Actor-1: JSON string of {name,type}[], Part-2 UI will parse
...
interface Window {
  api: {
    ...
    comments: {
      ...
      add:    (c: { task_id: string; author_id: string; author_name: string; content: string; task_title?: string; assignee_emails?: string[] }) => Promise<import('./types').TaskComment>
    ...
    intelligence: {
      ...
      analyzeText:          (opts: { task: 'relevance' | 'reconcile'; text: string; projectConfig?: { name?: string; keywords?: string; scope?: string } | null; userNotes?: string | null; existingTags?: string[]; priorAi?: Record<string, unknown> | null }) => Promise<{ ok: true; result: { relevance_score?: number; relevance_reasoning?: string; summary?: string; suggested_tags?: string[]; article_type?: string; capabilities?: Array<{ system: string; actor?: string; actor_type?: string; cost?: string; category?: string; relationship?: string }>; key_facts?: Array<{ label: string; value: string }>; proposed_sections?: Array<{ section: string; confidence: 'high' | 'medium' | 'low' }>; channel?: 'state-procurement' | 'commercial-retail' | 'n/a'; routing_reasoning?: string; subject_countries?: string[]; mentioned_countries?: string[]; actors?: { name: string; type: string }[] } } | { ok: false; error: string }>
      ...
      updateSocialFields:   (id: string, patch: Record<string, any>) => Promise<{ ok: boolean }>
      saveAiAnalysis:       (id: string, ai: { relevance_score?: number; relevance_reasoning?: string; summary?: string; suggested_tags?: string[]; article_type?: string; capabilities?: unknown[]; key_facts?: unknown[]; proposed_sections?: Array<{ section: string; confidence: string }>; channel?: string; routing_reasoning?: string; subject_countries?: string[]; mentioned_countries?: string[]; actors?: { name: string; type: string }[] }) => Promise<{ ok: true; ai: { relevance_score?: number; relevance_reasoning?: string; summary?: string; suggested_tags?: string[]; analyzed_at: string }; routing?: { proposed_sections: Array<{ section: string; confidence: string }>; channel: string; routing_reasoning: string; proposed_at: string } } | { ok: false; error: string }>
      ...
      setRoutingConfirmed:  (id: string, sections: string[]) => Promise<{ ok: boolean; routing?: unknown; error?: string }>
      setIncidentFlag:      (id: string, value: boolean | null) => Promise<{ ok: boolean; human?: unknown; error?: string }>
      ...
      createTag:            (name: string, type: string, boardId: string) => Promise<{ ok: boolean; name: string; error?: string }>
