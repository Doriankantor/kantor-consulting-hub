  ...
  return (
    <div className="flex items-center gap-1.5 px-4 py-2">
      {status === 'syncing' && (
        ...
        <span className="text-[10px] text-white/25">Syncing to Drive…</span></>
      )}
      {status === 'synced' && (
        <><span className="w-1.5 h-1.5 rounded-full bg-green-500 shrink-0" />
        <span className="text-[10px] text-white/25">Drive synced</span></>
      )}
      {status === 'error' && (
        <><span className="w-1.5 h-1.5 rounded-full bg-red-500 shrink-0" />
        <span className="text-[10px] text-white/25">Sync error</span></>
      )}
    </div>
  )
}
