...
--------------------------------------------------------------------------------
> RESUME HERE (CURRENT, 2026-08-12 -- DATA-MODEL VERDICT IN (Cowork): the two-shape split is an ARTIFACT, not essential -> the unified card sits on ONE shape. S0 DONE (20e12de) - getSourcePipeline SELECT widened, pipeline row now carries full source core on one shape. S1 DONE (a54082c) - card home src/renderer/src/components/source-card/ built (parse/sourceCore/SourceCard), read-only card mounted on News. S2 DONE (e613c52) - geography + actors editing wired through the card; S1 always-amber regression fixed (cue clears on touch); confidence/tags/relevance stay live in NewsTab quick-controls. S4 DONE (96a3da5) - incident control lifted onto News via SourceCard; News card now behaviorally COMPLETE. S5 DONE (460f79d) - shared SourceCard mounted read-only on Social (first cross-type mount); SourceCard hardened with general data-driven empty guards (title/relevance suppressed when absent; geo/actor chips shown only with data OR a real edit handler) so ungated/structure-sparse rows render no misleading blanks; duplicate confidence/status/date stripped from the Social header. Overflow "...+N" popovers FOLDED into the later REDESIGN PHASE (they group zones that do not exist yet)).

S6 DONE (a6dd0aa) - SourceCard mounted read-only on Documents. Option A (card title owns the filename; doc rows set title=file_name): stripped the header filename line + confidence/status badges; KEPT byline+date (added_at) because docs have no published_at. Added a GENERAL publishedAt empty-guard to SourceCard ({core.publishedAt && ...}, S5-style, NOT type-aware) so the empty doc date span is suppressed; Social/News dates still render (verified in-app). Dead constants conf/confStyle/CONFIDENCE_COLORS/STATUS_COLORS removed from DocumentsTab.

S7 DONE (cdc8461) - SourceCard mounted read-only on Interviews. Docs-style (interview rows set title = interview title or 'Untitled interview'): Option A, card title owns it; stripped header title line + status badge; KEPT byline+date (added_at). STATUS_COLORS removed. Interview published_at = interview date (optional); header date is added_at (creation), a different field, so no dup. Zero new SourceCard code for the mount itself.

B DONE (confidence-correctness cleanup surfaced by S7) - split B1/B2:
  B1 (1c5f534) - News hand-add now REQUIRES a confidence pick (added to submit validation + red-border error surface + 'Confidence *' label, matching the title/project pattern). Closes the only path that wrote null-confidence article rows. NEWS_EMPTY_FORM.confidence stays '' (so the user must actively pick).
  B2 (67791f7) - SourceCard confidence badge now GUARDED ({core.confidence && ...}) instead of always-rendering NULL->'low'. Suppresses the spurious 'LOW' on null-confidence interviews; news/social/document badges still render (all non-null). Verified in-app across all four types.
Safety basis for B2 (authoritative): Supabase cloud count this session showed article 0 null / social 0 null / document 0 null / interview 1 null. Only interviews are null and SHOULD stay null (no confidence concept). No backfill was needed. B1 closed the null-News hole BEFORE the guard shipped.

PHASE A COMPLETE - shared SourceCard now mounted read-only on all four source types (News, Social, Documents, Interviews) with correct data-driven empty guards and no spurious badges.

SLICE 0 DONE (06942ba restored the artifacts; spec-trim + pointer-fix land in this closing docs commit) - the lost unified-card design artifacts are RESTORED TO VERSION CONTROL. The originals were never truly lost, only never committed: both were recovered from ~/Downloads and re-homed under docs/. Now committed: the approved mockup (docs/unified-source-card-mockup.html, the newer 722-line variant) and the model spec (docs/unified-card-model-spec.md, with the drone-reuse mapping demoted to a parked out-of-scope appendix; sections 1-4 are the KC Hub spec). The previously dangling outputs/ pointers in this file are repointed to docs/. The design no longer lives only on local disk. NOTE: 06942ba committed the spec UNTRIMMED and did NOT actually touch this file despite its commit message - the trim and the pointer repointing are in the closing docs commit alongside this note.

SLICE 1 DONE (b52e66a) - SourceCard restructured from flat column into the unified shell: TITLE SURFACE (hero title + snippet-as-summary-slot + kicker) -> THREE TINTED ZONES (A cogwheel=assessment/routing: relevance/language/origin/sections/incident; B globe=geography: gate-geo chip + GeographyChips; C bust=engaged entities: ActorChips) -> collapsible REVIEW & ANNOTATE footer (shell only). Pure read-only re-skin of ONE shared component; verified in-app on all four intel tabs (News editable incl. amber AI-unconfirmed cue intact = props survived; Social/Docs/Interviews suppress empty zones - no orphaned tinted boxes).

FOUR DECISIONS LOCKED THIS SLICE (later slices depend on these):
- Kicker OWNS status + confidence (title surface); Zone A does NOT repeat them.
- Snippet IS the summary slot (same field, not a separate one).
- {children} (News project-selector + gate-error) relocated into the Review & Annotate footer - approval-readiness near its future gate.
- Empty-zone suppression is ZONE-LEVEL, data/handler-driven, never type-keyed: a zone renders only if it has content OR a live edit handler for its axis.

DEFERRED WITH INTENT (do NOT lose - build in the named later slice):
- AI-REPLACES-SNIPPET: snippet currently renders the GDELT pull as-is; the destination is that AFTER AI analysis, snippet is replaced by a short AI summary of the piece. This is a WRITE-PATH behavior (analysis step overwrites snippet / writes summary) - belongs in the AI-analysis slice, NOT the re-skin. Logged here so it isn't lost.
- CLICK-TO-OVERRIDE PILLS (Slice 3): confidence + relevance are wrapped in zero-box display:contents swap-point spans so Slice 3 turns them interactive with ZERO relayout. They render as READ badges today.
- EDITABLE GEO/ACTORS EVERYWHERE (S5b): only News edits geo/actors today (only it has the write path). Making Social/Docs/Interviews editable = new write paths + freeform->structured migration + SourceCore change. The zone add-affordances light up automatically once a live handler is passed - the shell is already built to receive them.

KNOWN STATE: the Review & Annotate footer is currently NEWS-ONLY (rendered under `children &&`; the other three tabs pass no children so the footer does not render). This is correct for Slice 1 (nothing in the footer for them yet); the render condition widens when notes/annotation land as their own slice.

PRINCIPLES BANKED THIS ARC:
- TITLE slot is populated ONLY for types with a real name/headline concept: News (article headline), Docs (filename), Interviews (interview title). Structural-identity types leave it dark: Social (handle+platform). The dark social title is CORRECT BY DESIGN, not a gap - forcing content into it would duplicate the content preview.
- CONFIDENCE stays null for types with NO confidence concept (interviews); the card guard correctly suppresses the badge rather than inventing a 'low'. Types that SHOULD carry confidence must never be null (enforced at the write form).
- SourceCard empty-guards are GENERAL and data-driven, never type-aware (the no-type invariant holds): language, snippet, publishedAt, confidence, geo, actors, relevance, title all render only on real data or a real edit handler.

SLICE 2a DONE (b74fee1) - removed the ActorChips .slice(0,20) truncation cap in dedupe(). This was a latent WRITE-PATH data-loss bug (reads showed all actors, but the first edit of a >20-actor source permanently dropped #21+ from storage). Cloud check confirmed MAX actors on any source = 15, zero rows at/over 20 -> no data was ever clipped; the fix is purely preventive. Dedupe still de-dupes (uniqueness via the lowercased-name Set, untouched). The `actors` column ([{name,type}] JSON on intelligence_sources) is the structured card field, distinct from social's free-text actors_mentioned.

SLICE 2b DONE (07a1ee7) - overflow "...+N" grouped popovers, 7 files. New shared hook src/renderer/src/hooks/usePopoverDismiss.ts (lifted from Todo.tsx's module-private copy; Todo's 3 call sites now import it). New OverflowPopover component + actorGroups helper (both in pages/Intelligence/). Actor grouping is DATA-DRIVEN by the real `type` field into 5 groups (VNSAs/State forces/Industry/Extra-regional/Unknown, empty groups skipped) - NOT the mockup's name-regex. Geography overflow is MENTIONED-only; SUBJECT stays uncapped. MAX=5 inline per overflowing zone; pill in zone accent (violet actors / teal mentioned); popover anchored via the TagPicker portal pattern (createPortal + position:fixed + getBoundingClientRect, viewport-clamped, flips up if no room). Both chip components got an additive `editable` prop (editable={!!onActorsChange} / editable={!!onCountriesChange} from SourceCard) - flat chips on read, removable+add on edit. Verified in-app: News editable (pills open/toggle clean, 5 groups, removable, add works) + read-only tabs (flat chips, no x/add). All existing chip handlers preserved verbatim; renderer-only, zero write-path.

ONE DEVIATION (accepted, Option A): the editable ADD control sits BESIDE the "...+N" pill, NOT inside the popover. The existing add uses component-level state (adding/value in ActorChips, edit in GeographyChips); a second in-popover add-input would share that state (bug) or force a handler refactor (violates preserve-verbatim). Add capability is fully present, just positioned beside the pill. An independent in-popover add-input is a clean future follow-up if wanted.

SLICE 2b DECISIONS (banked - all HONORED in 07a1ee7; kept for the build rationale):
- Actor grouping is DATA-DRIVEN by the real `type` field (VNSA/state/extra-regional/commercial/unknown), NOT the mockup's name-regex (which existed only because mock data was bare strings; porting it would regress). FIVE groups: VNSA->VNSAs, state->State forces, commercial->Industry, extra-regional->Extra-regional, unknown->Unknown.
- Overflow is MENTIONED-only; SUBJECT geography stays uncapped (primary axis, always shown in full). MAX=5 inline per overflowing zone, rest behind a "...+N" pill in the zone accent colour -> zone-anchored popover grouped by sub-type.
- Add an `editable` prop to the chip components (editable={!!onActorsChange} / editable={!!onCountriesChange}) so the popover shows flat chips on read vs removable+add on edit. Additive prop, NOT an edit-path change.
- Anchoring uses the codebase's PORTAL pattern (TagPicker: createPortal + position:fixed + getBoundingClientRect), NOT the mockup's absolute-in-card math (Slice-1 zones have no position:relative and risk clipping). Lift usePopoverDismiss (currently module-private in Todo.tsx) to a shared hook for click-outside/Escape.
- Renderer-only: SourceCard + GeographyChips + ActorChips + a popover component/CSS. Zero write-path, zero data change.

SLICE 3 DONE (0530dd6) - click-to-override CONFIDENCE pill on SourceCard. New ConfidencePill component (mirrors IncidentChip's editable/read shape; read branch byte-identical to the old badge); new additive onConfidenceChange? prop threaded from NewsTab to the EXISTING handleConfidence -> updateConfidence writer (sets confidence_override=1). Renderer-only, 3 files, no new writer/IPC/schema. Interactivity gated purely on onConfidenceChange presence (no type-keyed logic): News edits, read surfaces show flat badge. Verified in-app.

SLICE 3b DONE (8e54b1b) - relevance override fold-pill with PICKER. New RelevancePill component: when core.humanRel is set, folds the human High/Med/Low category IN PLACE OF the AI REL n badge, marked as a human override (distinct indigo family + "H" marker) with a tooltip surfacing the AI score ("Researcher override - AI scored REL n") so provenance is never hidden. Interactive affordance is a POPOVER PICKER (not a cycle): High/Med/Low set in one click, "Use AI score" clears in one click (choose(null)) - clearing NEVER transits through intermediate values (no stray writes). Current selection is checkmarked. Reuses the Slice-2b popover infra (usePopoverDismiss + createPortal/position:fixed/getBoundingClientRect anchoring). Wires to the EXISTING setHumanRelevance writer via handleHumanRelevance (null round-trips through NewsTab's v??'' -> handler's ''||null). The pill NEVER writes relevance_score - only analysis_json.human.relevance (gate-safe separation preserved). Renderer-only, 3 files, no new writer/IPC/schema. Read surfaces show the folded human pill flat (marker+tooltip intact, not clickable). Verified in-app.

VISUAL REDESIGN STATUS: the unified-card visual rollout is now complete except Slice 4. Shipped: Slice 1 (zoned shell) + 2a (actor cap fix) + 2b (overflow popovers) + 3 (confidence pill) + 3b (relevance fold-pill). The card face now owns confidence AND relevance editing on the Intel surface.

KANTOR FRAMEWORK - what it is (learned this session): reference/provenance anchors, NOT collected intelligence. Four seeded rows (the two FIU/Jack D. Gordon Institute "Contested Skies" foundational reports + two Kantor Consulting own pages), identified by added_by_name === 'Kantor Framework' (a string match, no type/flag), seeded pre-approved with confidence LOCKED (confidence_override=1, status=approved), geography sentinel 'Framework' (not a real country), EXCLUDED from the AI gate, and FILTERED OUT of the News list (NewsTab.tsx:473). Distinct from 'Contested Skies Archive' (fixed=false in the same seed -> normal re-gradeable articles). Because framework rows never reach a News SourceCard, the confidence pill has NO live exposure to them.

PHASE-B GUARD NOTE (do when it becomes live, not before): when Phase B converges editable SourceCards onto New Sources / Pre-Commit / All Sources, CONFIRM those surfaces filter framework refs like News does, OR add a framework guard to handleConfidence (no-op on added_by_name==='Kantor Framework'). Deferred deliberately.

B DONE (6090b2c) - retired the redundant legacy footer confidence + relevance <select> controls in NewsTab (card pills own both edits now); dropped the orphaned humanRel local. handleConfidence + handleHumanRelevance retained (card pills call them). Renderer-only, net -35 lines.

ROUTING RENAME DONE (38203c4) - the SourceCard Slice-1 footer shell was mislabeled "Review and annotate", colliding with NewsTab's original human-layer footer of the same name. Renamed the CARD footer to "Routing" (holds project selector + gate error = approval routing), pencil icon -> arrow. NewsTab's human-layer footer keeps "Review and annotate". Collision resolved (the label now appears once).

NEXT = SLICE 4 - ROUTABILITY GATE + completeness meter. The approve-gate audit (done this session, read-only) found the real News approve gate is THIN - project + topic only, client-side (NewsTab canApprove = projectSel !== '' && themaTags.length > 0); the cloud writer (intel.ts updateStatus) enforces NOTHING beyond a phantom-row existence guard - a flagged client/server gap. DECISION: TIGHTEN the gate into a real routability gate, and the completeness meter mirrors it (ONE source of truth - the meter reads the same canRoute the button enforces, or it lies).

THE GATE SPLITS ACROSS TWO STAGE-TRANSITIONS:
- GATE 1 (Intel -> New Sources, the "Approve/route" action on the card, ALL FOUR types news/social/document/interview): SIX checks -
    * CONTENT present (per-type: news = article text, doc = file content/notes, interview = transcript/notes, social = PASTED DESCRIPTION of the post)
    * AI ANALYZED (analysis has run - universal hard gate; analysis is the step that sets a source up for routing, all types)
    * TAGS (>=1 thematic tag, all types)
    * CONFIDENCE CONFIRMED (confidence_override === 1 - researcher confirmed/overrode via the pill, all types)
    * GEOGRAPHY CONFIRMED (researcher actively set it - a country OR REGIONAL OR GLOBAL all count; untouched/default does NOT. Confirmation gesture = clicking/adding a geography chip, shown with an H marker like relevance)
    * INCIDENT CONFIRMED (AI pre-determines is-incident/not; researcher CONFIRMS by clicking the incident control - purple+H if incident, or the click marking not-incident; either direction is confirmation)
  RELEVANCE is NOT a gate (AI score stands). NO SECTIONS at Gate 1 - sections are AI-PREVIEW only at Intel.
- GATE 2 (New Sources -> downstream, the EXISTING stage gate per spec): >=1 SECTION finalized ("a source cannot leave New Sources until a section is selected"). Section FINALIZATION happens at New Sources, NOT Intel (preserves the locked "sections editable only at New Sources" rule). At New Sources geography is READ-ONLY - changing it requires MOVE BACK TO INTEL (reciprocal rule). Gate 2 is Phase-B / pipeline-surface work, LATER.

The card lives on Intel, so its meter mirrors GATE 1 (the route hop that actually happens on this surface). Sections are not the card meter's concern.

THE CRUX / OPEN SCOPE QUESTION (the Slice-4 diagnose resolves it): four checks are clearly backed today - content (has text), AI (analysis ran), tags (has tags), confidence (confidence_override flag exists). The TWO crux checks are GEOGRAPHY and INCIDENT: the gate needs a PERSISTED "human confirmed" signal DISTINCT from the VALUE (so it can tell "researcher confirmed REGIONAL" from "AI proposed / untouched default"). The H-marker is the intended UX (like relevance's human.relevance). UNKNOWN until the diagnose: do geography + incident already PERSIST that confirm signal (SourceCore carries geographyConfirmed, and incident state - verify whether either encodes a human-confirm distinct from the AI proposal), or must a small confirm-write be ADDED (making Slice 4 = a tiny write-path prerequisite sub-slice FIRST, then the renderer meter)? Confidence is the model - it has confidence_override. Value availability is otherwise fine: SourceCore already carries thematicTags, dispositionTags, region, incident, proposedSections, confidence, gateProcessed.

SLICE 4 PROGRESS (2026-08-17):
- 4a DONE (3b1fe17): the CRUX above is resolved. Geography now persists an IMPLICIT human-confirm via geography_confirmed (any countries edit on the card flips it to 1, ADDITIVE to the lists write, never on AI-proposal/load); SourceCore surfaces hasContent + confidenceOverride. Cloud-verified.
- 4b (meter + gate) SPLIT into two: 4b-1 (the 6-dot RoutabilityMeter + canRoute single-source-of-truth + content-dot fix + click flyout; FIVE checks live, incident dot DIM) and 4b-2 (the incident control, below). 4b-1 gates the Approve button on the interim FIVE live checks, NOT canRoute.all, until 4b-2 lands.

NEW BACKLOG (queued off Slice 4):

SLICE 4b-1b (SUPERSEDED - the earlier "no-forks clean uniform build / all six checks completable on all types" framing was WRONG; cross-type meter gating is now BLOCKED on S5b) - extend the routability meter to social/document/interview + flyout scroll-dismiss. The real finding + decision:
- FINDING: 4b-1b is NOT a clean uniform build. The geography editor (GeographyChips/updateCountries) and the confidence pill exist ONLY on NewsTab. Social/Document/Interview cards render those zones but they are DISPLAY-ONLY - no edit path, so geography_confirmed and confidence_override can never be set on those types. Result: the geography + confidence dots would be PERMANENTLY DIM there, making their meters un-completable and the route gate unsatisfiable on 3 of 4 types.
- ROOT CAUSE: editable geo/actors/confidence on non-News types is S5b - the deferred write-path arc (new write paths + freeform->structured migration + SourceCore change). The unified-card VISUAL redesign is done; the unified-card EDITING was never uniformly built. The meter made this gap consequential (a gate you can't satisfy).
- DECISION (Dorian): PULL S5b FORWARD. Do not ship an informational-only meter; instead build editable geo/actors/confidence on all types (S5b), THEN complete 4b-1b's cross-type gating. Resequences S5b ahead of the publication pipeline. "Finishing the cards" now explicitly includes S5b.
- NEXT: S5b SCOPING DIAGNOSE (read-only) before any build - map freeform-vs-structured storage per type, the migration size/risk, and the write paths + SourceCore changes needed. S5b contains a LIVE-DATA MIGRATION (freeform geo/actors -> structured) = highest-stakes change class; diagnose exhaustively before building, build fresh not tired.
- SCROLL-DISMISS (the OTHER half of 4b-1b) is INDEPENDENT of S5b and still stands: add to the shared usePopoverDismiss (2b overflow popovers use a DIFFERENT dismiss mechanism, so no collateral), dismiss the meter flyout on scroll. Can ship on its own; the cross-type gating cannot.
- 4b-1b's cross-type meter gating is BLOCKED on S5b and resumes after it.

SLICE 4b-2 (queued, right after 4b-1) - INCIDENT ROUTE-GATE CONTROL. Discovery: the gate/AI does NOT pre-determine incident, so the intended "AI pre-determines, researcher confirms" model doesn't work (there's nothing to confirm - a researcher had to fake-toggle mark-incident then un-mark to get a confirmed state). DECISION: replace with an explicit two-way "Incident? Yes / No" control the researcher answers directly; the incident routability dot lights when answered EITHER way (yes or no), persisted as a human decision (no AI-determination dependency). Needs: the Yes/No control (replacing "+ Mark as incident" one-way), its persistence, and wiring canRoute's incident check + flipping the button gate from the interim 5-check to canRoute.all. 4b-1 ships the meter with the incident dot DIM/pending until this lands.

SNIPPET/SUMMARY NOT UPDATED BY AI (queued, separate diagnose, adjacent to ARC II AI pipeline): discovered live - GDELT provides a one-line snippet that is SUPPOSED to be replaced by a substantive AI summary after analysis, but running AI analysis does NOT update the snippet (the AI-replaces-snippet behavior was banked as deferred and is confirmed UNBUILT). Current snippet is "largely correct" but wants a more substantive post-AI version. Investigate whether saveAiAnalysis writes a summary field and wire snippet <- AI-summary. NOT part of the card/meter work - it's AI-analysis write-path work. (Note: 4b-1's content dot checks the REAL article body in `content`, NOT the snippet - notesText(content).length>40 - so the vacuous-snippet problem does not affect the gate; this backlog item is about the DISPLAYED snippet quality, a separate axis.)

DECIDED SEQUENCE (Dorian's call this session):
- AFTER Slice 4: FOOTER-LAYOUT REDESIGN (design-first/mockup). The News card footer is three stacked border-t bands (Routing footer + unlabeled topic/actions row + Review-and-annotate human-layer footer) = too tall; the topic/actions row is orphaned (no header). Deferred to after Slice 4 DELIBERATELY because Slice 4 adds the meter INTO this region - redesign once, with all final footer contents present.
- AFTER the footer redesign, the CARD VISUAL REDESIGN IS DONE.
- THEN PIVOT TO THE PUBLICATION PIPELINE (ARC II - the geography/country-default model, "the next MAJOR priority"). This is the decided next major arc after the card work closes.
- Separable card-capability arcs (S5b editable geo/actors everywhere, A2 write-propagation, Phase B PipelineSourceCard convergence) remain deferred and are NOT part of "finishing the cards" - they come after the publication pipeline unless re-prioritized.

ON THE HORIZON - IMAGE ATTACHMENTS (scoped future arc, design-first, rides with/after ARC II publication pipeline): let researchers attach an image to a source (from an article / social post / document) via an "add image" field, and show a clickable image indicator on the published Contested Skies monitor. Feasible - SourceCore already carries imageUrl (URLs work today); the real new piece is IMAGE-BYTE STORAGE (Supabase Storage bucket + upload path) for drag-dropped files, plus per-type capture UI, a one-vs-many data-model decision (image_url column vs a source_images table), carry-through in content.json at publish, and the clickable render on the monitor SPA. ~3-5 slices. Deferred to ARC II because the display half IS publication-pipeline work (rides in content.json); building capture before the pipeline carries it would store images unshown.
...
REFINED ARC SEQUENCE (updated by the verdict):
  S0 [DONE 20e12de]: widen getSourcePipeline SELECT to carry the full source core + extend InfoPageSourceRow type.
...
CARD DESIGN + EDITABILITY MATRIX: LOCKED (unchanged) -- see prior block / docs/unified-source-card-mockup.html
