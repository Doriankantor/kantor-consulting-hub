...
NEXT = SLICE 3 - click-to-override confidence/relevance pills. The Slice-1 kicker confidence badge + Zone-A relevance badge are already wrapped in zero-box display:contents swap-point spans, so Slice 3 swaps badge->interactive pill with minimal relayout. It wires to the EXISTING updateConfidence writer (no new write path) - confirm in the diagnose whether relevance has an equivalent writer or is override-only. THEN Slice 4 - completeness meter, which REQUIRES a read-only approve-gate audit FIRST so the meter's "ready" state equals the real commit gate exactly (the meter must never lie). Separable later arcs unchanged: S5b (editable geo/actors everywhere + freeform->structured migration), A2 (write-propagation), Phase B (converge PipelineSourceCard on New Sources/Pre-Commit/All Sources).
