...
NEXT = SLICE 3b - relevance override pill. DECISION LOCKED: Option 2 (FOLD) - when a human relevance is set, the pill REPLACES the AI REL n badge visually (human High/Med/Low where REL n was). BUILD CONSTRAINT: the fold must PRESERVE PROVENANCE - a human override must be visibly marked as a human call (marker/tooltip) with the AI score still discoverable, NOT silently shown as if it were the AI's own output. Writer exists: setHumanRelevance(id, value|null) -> analysis_json.human.relevance (gate-safe, never touches relevance_score); NewsTab calls it via handleHumanRelevance; core.humanRel carries the value. So 3b is renderer-only (SourceCard + a NewsTab onRelevanceChange? thread) + the fold display - NOT S5b.

DECISION LOCKED (B) - LEGACY FOOTER CLEANUP: do NOT retire the NewsTab footer quick-controls piecemeal. When 3b ships the relevance fold-pill, retire BOTH footer <select> controls together in one cleanup - the confidence select (~NewsTab.tsx:1526) AND the human-relevance override select (~1562) - since the card face will then own both. Until then both stay (harmless: each edits the same value as its card pill via the same handler). NOTE the confidence select is a CONDITIONAL block (gradeable select vs "Authoritative - fixed" for framework refs) - removal must not leave a dangling branch; diagnose the footer block before cutting.

THEN Slice 4 - completeness meter, which REQUIRES a read-only approve-gate audit FIRST so the meter's "ready" state equals the real commit gate exactly (the meter must never lie). Separable later arcs unchanged: S5b (editable geo/actors everywhere + freeform->structured migration), A2 (write-propagation), Phase B (converge PipelineSourceCard on New Sources/Pre-Commit/All Sources - see guard note above).
