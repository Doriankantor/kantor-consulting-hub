...
SLICE 2b DECISIONS LOCKED (ready to build - full read-only diagnose already done):
...
NEXT = SLICE 2b - build the overflow "...+N" grouped popovers per the locked decisions above (diagnose complete; go straight to build). THEN Slice 3 (click-to-override conf/rel pills - swap the contents-wrapped badges), Slice 4 (completeness meter - AFTER a read-only approve-gate audit so "ready" cannot lie). Separable later arcs unchanged: S5b (editable geo/actors everywhere + freeform->structured migration), A2 (write-propagation), Phase B (converge PipelineSourceCard on New Sources/Pre-Commit/All Sources).
