...
NEXT (pick one): S5b (editable structured geo/actors on Social - needs new write paths + freeform->structured migration + a SourceCore change; DEFERRED, own arc); OR the boxed-zone redesign (overflow '...+N' popovers folded here). Both are future card arcs, not continuations of Phase A.
