...
NEXT = B - LEGACY FOOTER CLEANUP (precondition now met - both card pills exist). Retire BOTH NewsTab footer <select> controls together: the confidence select (~NewsTab.tsx:1526) AND the human-relevance override select (~1562), since the card face now owns both edits. DIAGNOSE FIRST (read-only): the confidence select is a CONDITIONAL block (gradeable select vs "Authoritative - fixed" for framework refs) - map the full footer block before cutting so removal leaves no dangling branch or orphaned handler/variable. Confirm handleConfidence + handleHumanRelevance stay (the card pills still call them); only the footer <select> UI is removed. Renderer-only.

THEN Slice 4 - completeness meter, which REQUIRES a read-only approve-gate audit FIRST (the meter's "ready" state must equal the real commit gate exactly, or it lies). After Slice 4 the visual redesign is DONE. Separable later arcs unchanged: S5b (editable geo/actors everywhere + freeform->structured migration), A2 (write-propagation), Phase B (converge PipelineSourceCard - carries the framework-ref guard note).
