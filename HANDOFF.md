...
NEXT = SLICE 1 - boxed-zone layout, read-only, all surfaces. Restructure SourceCard's flat column into the title surface + three tinted zones (cogwheel=assessment/routing, globe=geography, bust=engaged entities) + collapsible Review & Annotate footer, per docs/unified-card-model-spec.md + docs/unified-source-card-mockup.html. READ-ONLY re-skin only: NO new edit axes, NO completeness-meter gate logic yet (the meter needs a separate read-only approve-gate audit so 'ready' can't lie), NO overflow popovers yet (Slice 2). Renderer-only. Then Slice 2 (overflow ...+N popovers), Slice 3 (click-to-override conf/rel pills, wires to the existing updateConfidence writer), Slice 4 (completeness meter, after the approve-gate audit). SEPARABLE LATER ARCS (not required for the intel-tab redesign): S5b (editable structured geo/actors on Social - write paths + freeform->structured migration + SourceCore change), A2 (write-propagation), Phase B (converge PipelineSourceCard on New Sources/Pre-Commit/All Sources).
