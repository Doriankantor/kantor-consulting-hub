...
NEXT = SLICE 2 - overflow "...+N" grouped popovers. Each zone caps ~5 chips inline, rest collapse behind a "...+N" pill in the zone accent colour; click opens a zone-anchored popover with the full set GROUPED by sub-type (actors -> VNSAs/State/Industry; geography -> subject/mentioned). Presupposes the Slice 1 zones (which now exist). Renderer-only, per docs/unified-card-model-spec.md section 4 + the mockup. THEN Slice 3 (click-to-override conf/rel pills - swap the contents-wrapped badges), Slice 4 (completeness meter - AFTER a read-only approve-gate audit so "ready" cannot lie). Separable later arcs unchanged: S5b (editable geo/actors everywhere + migration), A2 (write-propagation), Phase B (converge PipelineSourceCard on New Sources/Pre-Commit/All Sources).
