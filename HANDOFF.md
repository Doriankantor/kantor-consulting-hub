  ...
  *** INCIDENTS ARC COMPLETE *** All four slices done: Slice 1 generate (analyze.ts incident task,
  ...
  • KNOWN ITEMS (non-blocking):
    - event_date defaults to GENERATION date when a source gives no precise incident date (Catatumbo got
      ...
      timelines; consider storing source's stated period or null.
...
▶ RESUME HERE → next candidates, in order:
  (a) TWO SMALL TUNING ITEMS (kept separate, each its own commit, both analyze.ts):
      - event_date DEFAULT fix: incident date falls back to GENERATION date when the source gives no precise
        date (Catatumbo shows today's date) -- for the temporal/mappable incident dataset this corrupts
        timelines; store the source's stated period or null instead (analyze.ts / normalizeIncident).
      - NARRATIVE-LEAK PROMPT PASS (see KNOWN -- NARRATIVE LEAK above): one integrate-prompt pass barring
        event-statistics / casualty-tallies from narrative now that incidents capture them.
  (b) P4b granular per-hunk accept.
  (c) P4c card proposals in review (extend the accept flow to cards).
  (d) INCIDENT acceptance (narrative-first lean: after the narrative spine, now proven, extend accept to the
      incident card).
