...
NEXT = SLICE 4 - completeness meter. GATED: a read-only APPROVE-GATE AUDIT must run FIRST - the meter's "ready" state must equal the real commit/approve gate EXACTLY, or the meter lies. Build the meter only after the audit confirms what the real gate checks. The meter lives in the card's footer region (the "Routing" footer / Review & Annotate area). Renderer-only once the gate logic is known.
